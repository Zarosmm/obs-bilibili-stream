...
public:
    ...
    ~BilibiliStreamPlugin() {
        ...
        if (config.rtmp_code) {
            free(config.rtmp_code);
            config.rtmp_code = nullptr;
        }
        if (config.rtmp_addr) {
            free(config.rtmp_addr);
            config.rtmp_addr = nullptr;
        }
    }
...
//
	...
	struct Part {
	    int id;
	    QString name;
	    std::vector<json11::Json> list; // 子分区数组
	};
	std::vector<Part> parts;

	auto partition_data = bili_get_partition_list(); // 传 cookie 或 nullptr
	if (partition_data.is_array()) {
	    for (auto& item : partition_data.array_items()) {
	        Part part;
	        part.id = item["id"].int_value();
	        part.name = QString::fromUtf8(item["name"].string_value().c_str());
	        if (item["list"].is_array()) {
	            part.list = item["list"].array_items();
	        }
	...
	auto updateAreaCombo = [&](int part_index) {
	    areaCombo->clear();
	    if (part_index >= 0 && part_index < (int)parts.size() && !parts[part_index].list.empty()) {
	        const auto& areas = parts[part_index].list;
	        int selected_area_index = 0;
	        for (size_t i = 0; i < areas.size(); i++) {
	            int id_int = areas[i]["id"].int_value();
	            QString name = QString::fromUtf8(areas[i]["name"].string_value().c_str());
