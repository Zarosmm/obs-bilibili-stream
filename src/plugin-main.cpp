...
public slots:
	...
	void onStreamButtonTriggered()
	{
		...
		if (config.streaming) {
			...
			if (Bili::BiliApi::startLive(config, rtmp_addr, rtmp_code, message, face_qr, config.mid)) {
				...
				layout->addWidget(
					new QLabel(QString("Bilibili已开始直播请复制以下内容进行推流\n"
							   "若自定义推流失败请使用预设的B站推流更换推流地址尝试\n"
							   "RTMP 地址: %1\n推流码: %2")
							   .arg(rtmp_addr.c_str(), rtmp_code.c_str()),
						   dialog));
	...
	void onUpdateRoomInfoTriggered()
	{
		...
		QLabel *roomLabel = new QLabel(
			QString("直播间: https://live.bilibili.com/%1").arg(QString::fromStdString(config.room_id)),
			dialog);
		...
		if (partition_data.is_array()) {
			for (const auto &item : partition_data.array_items()) {
				parts.push_back({item["id"].int_value(),
						 QString::fromUtf8(item["name"].string_value().c_str()),
		...
		auto updateAreaCombo = [=](int part_index) {
			areaCombo->clear();
			if (part_index >= 0 && static_cast<size_t>(part_index) < parts.size() &&
			    !parts[part_index].list.empty()) {
				...
				for (size_t i = 0; i < parts[part_index].list.size(); ++i) {
					...
					QString name = QString::fromUtf8(
						parts[part_index].list[i]["name"].string_value().c_str());
		...
		connect(partCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
			[=](int index) { updateAreaCombo(index); });
		...
		connect(confirmTitle, &QPushButton::clicked, [=]() {
			...
			if (!new_title.empty() && Bili::BiliApi::updateRoomInfo(config, new_title, update_message)) {
				...
				showMessageDialog(QStringLiteral("直播间标题更新失败\n") +
							  QString::fromUtf8(update_message.c_str()),
						  "消息");
			}
		});
